...
__IO u8 ATCmd[100];
__IO u8 ATLEN = 0;
__IO MenuType CotrolVale = SYSEND,ProVale = SYSXX;
void PC_USART_IRQHandler(void)
{
    ...
    if(USART_GetFlagStatus(PC_USART,USART_FLAG_IDLE)!= RESET)
    {
        ...
        else if (!strncmp((const char *)ATCmd,"AT+CALID1",9))
        {
            CotrolVale =  SYSCALID1;
        }
        else if (!strncmp((const char *)ATCmd,"AT+CALCVN1",10))
        {
            CotrolVale =  SYSCALCVN1;
        }
        else if (!strncmp((const char *)ATCmd,"AT+ECUNAME",10))
        {
            CotrolVale =  SYSECUNAME;
        }
        else if (!strncmp((const char *)ATCmd,"AT+SETDS",8))
        {
            CotrolVale =  SYSSETDS;
        }
        else if(!strncmp((const char *)ATCmd,"AT+OFF",6))
        {
            CotrolVale =  SYSOFF;
        }
        else if(!strncmp((const char *)ATCmd,"AT+SPEEDS",9))
        {
            CotrolVale =  SYSSS;
        }
        else if(!strncmp((const char *)ATCmd,"AT+SPEEDE",9))
        {
            CotrolVale =  SYSSE;
        }
        else
...
__IO u8 TIM2Count = 0;
FlagStatus Flagspeedw = RESET;
u8 speedw[8] = {4,4*3,4*6,4*12,4*5,4*14,4*7,4*8};
u8 speedwc = 0;
void TIM2_IRQHandler(void)
{
    if (TIM_GetITStatus(TIM2, TIM_IT_Update) != RESET)
    {
        ...
        if(Flagspeedw == SET)
        {
            if(ProVale == SYS01 || ProVale == SYS02 || ProVale == SYS03 || ProVale == SYS04)
            {
                IRQVar.CANCPT->CANTXCMD[29].Data[4] = SpeedCount+speedw[speedwc++];
            }
            else if (ProVale == SYS05 || ProVale == SYS06)
            {
                IRQVar.KCPT->KTXCMD[30].data[6] = SpeedCount+speedw[speedwc++];
            }
            else if (ProVale == SYS07)
            {
                IRQVar.KCPT->KTXCMD[30].data[6] = SpeedCount+speedw[speedwc++];
            }
            if(speedwc >= 7)
...
void K_LINE_IRQHandler(void)
{
    ...
    if (USART_GetFlagStatus(KL_LINE,USART_IT_RXNE)!= RESET)
    {
        ...
        {
            IRQVar.acctivecnt++;
            if(IRQVar.acctiveflag != 0)
            {
                IRQVar.acctiveflag = 0;
            }
            if( IRQVar.acctivecnt >= 3)
            {
                IRQVar.acctiveflag = 2;
            }
            else if(IRQVar.acctivecnt == 1)
            {
                IRQVar.acctiveflag = 1;
            }
            data = KL_LINE->DR;
            return;
        }
        else if ((IRQVar.LEN == 0 && KL_LINE->DR == 0x70) || (IRQVar.LEN == 0 && KL_LINE->DR == 0xF7))
        {
            IRQVar.flag = SET;
            IRQVar.CPS = 116;
            IRQVar.LEN = 0;
        ...
        if (IRQVar.ACCBYTE == RxRAM[IRQVar.LEN-1] && IRQVar.LEN > 3)	//
        {
            for (IRQVar.CPS = 0; IRQVar.CPS < IRQVar.KCPT->RXCMDCNT; IRQVar.CPS++)
            {
                ...
                if(IRQVar.flag == SET && IRQVar.KCPT->RXCMDCNT != IRQVar.CPS)
                {
                    IRQVar.LEN = 0;
                    break;
                }
